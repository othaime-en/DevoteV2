...
const CampaignDetails = () => {
      ...
      <div className="w-full flex md:flex-row flex-col mt-10 gap-[30px]">
        <div className="flex-1 flex-col">
          ...
          <div className="relative w-full h-[5px] bg-[#3a3a43] mt-2">
            <div className="absolute h-full bg-[#4acd8d]" style={{ width: `${calculateBarPercentage(state.startTime, state.endTime)}%`, maxWidth: '100%'}}>
            </div>
          </div>
        </div>
  ...
  <div className="mt-[20px] flex flex-col gap-4">
    {candidates.length > 0 ? candidates.map((candidate, index) => (
      <div key={candidate.candidateId} className="bg-[#1c1c24] p-4 rounded-[10px]">
        <div className="flex justify-between items-center">
          <p className="font-epilogue font-semibold text-[16px] text-white">{index + 1}. {candidate.name}</p>
          <p className="font-epilogue font-medium text-[14px] text-[#b2b3bd]">{candidate.role}</p>
        </div>
        <p className="font-epilogue font-normal text-[14px] text-[#808191] mt-2">{candidate.description}</p>
      </div>
    )) : (
